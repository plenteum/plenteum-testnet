...
std::string formatCents(const uint64_t amount)
{
    ...
    stream << std::setfill('0') << std::setw(WalletConfig::numDecimalPlaces)
           << amount;
    return stream.str().substr(0, WalletConfig::numDisplayDecimalPlaces); //only return the digits we want to display; //DL-TODO: Investigate whether this will limit display to only 2 decimal places
}
